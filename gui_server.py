...
#   (ex. "http://${OLLAMA_HOST:-localhost}:8000,http://localhost:8000").
...
class ConfigBody(BaseModel):
    ...
    persistent_agents: Optional[Dict] = None
...
async def run_engine_flow(objective: str):
    """
    [DÉPRÉCIÉ V9 AR-4] Orchestrateur legacy — remplacé par services.pipeline_service.run_engine_background().
    Conservé pour compatibilité ascendante éventuelle. Ne plus appeler directement.
    """
    global execution_state, _engine_instance
    ...
    # Enregistrement des outils standards
    ...
    # Enregistrement des outils de bac à sable Git (Git Safety)
    ...
    # [P5+P9] Enregistrement des outils Google Workspace (Calendar + Drive)
    ...
    # [Phase 2] Enregistrement des outils Gmail, Sheets, Tasks, YouTube, Contacts
    from tools.google_workspace import search_gmail, read_spreadsheet, get_tasks, search_youtube, get_contacts
    registry.register("search_gmail", search_gmail, "Recherche dans les emails Gmail. Syntaxe Gmail : 'from:X', 'subject:Y', 'is:unread', 'after:2026/05/01'. Args: query, max_results (défaut '5').")
    registry.register("read_spreadsheet", read_spreadsheet, "Lit les données d'un Google Spreadsheet. Args: spreadsheet_id (ID visible dans l'URL), range_notation (défaut 'Sheet1', ex: 'Sheet1!A1:D10').")
    registry.register("get_tasks", get_tasks, "Récupère les tâches Google Tasks en cours. Args: task_list_name (défaut '@default').")
    registry.register("search_youtube", search_youtube, "Recherche des vidéos YouTube. Args: query (termes de recherche), max_results (défaut '5').")
    registry.register("get_contacts", get_contacts, "Récupère les contacts Google (nom, email, téléphone). Args: max_results (défaut '10').")
    
    # [Phase 3] Enregistrement des outils Cloud TTS, Translation, Vision, STT
    ...
    from tools.minimax_tts import minimax_tts_synthesize
    registry.register("cloud_tts", cloud_tts_synthesize, "Synthèse vocale premium (Neural2/WaveNet). Args: text, voice ('neural2_female'/'wavenet_male'/etc), language (défaut 'fr-FR').")
    registry.register("minimax_tts", minimax_tts_synthesize, "Synthèse vocale expressive via MiniMax (clonage et émotion). Args: text, voice_id (défaut 'female-shaonv'), output_path (optionnel).")
    ...
    # [P6] Enregistrement de l'outil de génération d'images Imagen 4
    from tools.imagen import generate_image
    from tools.minimax_image import minimax_image_generate
    registry.register("generate_image", generate_image, "Génère une image via l'API Google Imagen 4 à partir d'un prompt textuel. Args: prompt (description), output_path (optionnel), model_variant ('fast'/'standard'/'ultra'), aspect_ratio ('1:1'/'16:9'/etc).")
    registry.register("minimax_image", minimax_image_generate, "Génère une image via l'API MiniMax (image-01). Args: prompt (description), output_path (optionnel), aspect_ratio (défaut '1:1').")
    
    # Initialisation du pont pour les serveurs MCP locaux
    ...
    # Création des agents built-in avec les modèles configurés
    ...
    antigravity_agent = AntigravityAgent(llm_gateway=gateway, tool_registry=registry, provider_name=config["antigravity_model"])
    ha_agent = HACommandAgent(llm_gateway=gateway, tool_registry=registry, provider_name=config["executor_model"])
    reviewer = ReviewerAgent(llm_gateway=gateway, provider_name=config.get("reviewer_model", "moyen"))
    
    # Moteur & Router
    session_id = f"gui_session_{int(asyncio.get_event_loop().time())}"
    token_tracker.init_session(session_id, objective)
    ...
    engine.register_agent(prompt_engineer)
    ...
    for custom in bridge.get_custom_agents_config():
        ...
        logger.info(f"[GUI_SERVER] Agent custom '{custom['name']}' enregistré.")
        ...
        for plugin_info in plugin_registry.discover():
            if plugin_info.enabled and plugin_info.agent_class:
                ...
                if plugin_agent:
                    ...
                    logger.info(f"[GUI_SERVER] Plugin agent '{plugin_agent.name}' enregistré.")
    except Exception as pe:
        logger.warning(f"[GUI_SERVER] Erreur de chargement des plugins : {pe}")

    
    # Callback pour intercepter les évènements de l'Engine et les diffuser en temps réel
    async def handle_engine_event(event_type: str, data: Any):
        execution_state["engine_state"] = global_state_to_dict(engine.state)
        if event_type == "orchestration_completed":
            execution_state["status"] = data["status"]
            if data["status"] == "error":
                execution_state["error_message"] = data.get("error_message", "L'orchestration s'est terminée sur une erreur.")
        
        # Pousser les tokens mis à jour à chaque transition
        try:
            from api.routes.billing import get_tokens
            tokens_data = get_tokens()
            await broadcast_event("tokens_updated", tokens_data)
        except Exception as te:
            logger.warning(f"Erreur de diffusion des tokens via SSE : {te}")
            
        # Diffusion asynchrone SSE de l'évènement d'origine
        await broadcast_event(event_type, data)
        
    engine.on_event = handle_engine_event

    # [P1-2.1] Router câblé (même si run_engine_flow est déprécié, on évite tout
    # Router nu résiduel dans le code).
    router = Router(default_agent="planner", llm_gateway=gateway, config=config)
    
    try:
        # Démarrage du pont MCP
        await mcp_bridge.start(registry, user_prompt=objective)
        
        # Enregistrement du début de session dans l'historique
        ...
        execution_state["engine_state"] = global_state_to_dict(final_state)
        
        # Enregistrement de la fin de session (succès)
        ...
        record_session_end(
            ...
            result_summary=str(final_state.history[-1].result_data)[:500] if final_state.history else None,
        )
        
    except Exception as e:
        logger.error(f"Erreur d'exécution de l'Engine: {e}")
        execution_state["status"] = "error"
        execution_state["error_message"] = str(e)
        execution_state["engine_state"] = global_state_to_dict(engine.state)
        
        # Enregistrement de la fin de session (erreur)
        try:
        ...
        _engine_instance = None
        # Fermeture propre du pont MCP et de tous les processus serveurs associés
...
async def websocket_tuning(websocket: WebSocket):
...
async def websocket_engine_state(websocket: WebSocket):
    await websocket.accept()
...
if __name__ == "__main__":
    import uvicorn
    # Lancement sur le port 8000 (rechargement optionnel avec filtres pour éviter les boucles infinies sur les écritures de cache/DB)
    # Écoute sur 0.0.0.0 pour permettre l'accès depuis le réseau local (Steam Deck, Freebox, etc.)
    reload_mode = "--reload" in sys.argv
    if reload_mode:
        # Nettoyer sys.argv pour qu'uvicorn ne lève pas d'exception sur les arguments non reconnus
        sys.argv = [arg for arg in sys.argv if arg != "--reload"]
        ...
        logger.info("[STARTUP] Lancement d'Uvicorn en mode standard (sans rechargement).")
