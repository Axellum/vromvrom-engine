...
class SandboxWrapper:
    ...
    async def execute(self, func_name: str, kwargs: dict[str, Any], agent_name: str = None) -> Any:
        """
        ...
        Args:
            ...
            agent_name: [#T233] Nom de l'agent appelant (permissions, cf. registre)
        ...
        return await self.registry.execute(func_name, kwargs, agent_name=agent_name)
    ...
    def set_agent_allowed_tools(self, *args, **kwargs):
        """[#T233] Proxy transparent vers ToolRegistry.set_agent_allowed_tools().

        Nécessaire car ExecutorAgent.invoke synchronise ses permissions à chaque
        invoke, même quand tool_registry pointe vers ce wrapper (mode sandbox)."""
        return self.registry.set_agent_allowed_tools(*args, **kwargs)
